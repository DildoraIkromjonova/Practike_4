...
const Modal = ({ typeData }) => {
  ...
  const getListData = async () => {
    dispatch({ type: "start", loading: true });
    try {
      let res = await fetch(`${api}/library`);
      res = await res.json();
      dispatch({ type: "success", payload: res, loading: false })
    } catch (error) {
      dispatch({ type: "error", loading: false })
    }
  }

  useEffect(() => typeData && setLoad(false), [typeData])
    ...
    <ModalContainer>
      ...
      <Form>
        ...
        <ButtonGroup>
          ...
          <SubmitBtn onClick={Postdata}>{payload ? load ? <Icon.Loading /> : "Edit" : "Submit"}</SubmitBtn>
        </ButtonGroup>
      </Form>
    </ModalContainer>
  )
